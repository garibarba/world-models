...
class MDRNNCell(_MDRNNBase):
    ...
    def __init__(self, latents, actions, hiddens, gaussians):
        super().__init__(latents, actions, hiddens, gaussians)
        self.rnn = mcLSTMCell([actions, latents, 1], hiddens, num_channels=1)
    ...
    def forward(self, action, latent, reward, hidden): # pylint: disable=arguments-differ
        ...
        if len(out_rnn) == 1:
            assert self.rnn.num_channels == 1
            out_rnn = 3 * out_rnn
...
class FMDRNNCell(MDRNNCell):
    ...
    def forward(self, action, latents, reward, hidden,
        ...
        mus, logsigmas, logpi, r, d, next_hiden = super().__call__(action, latent_input, reward, hidden)
